...
#      next if $album->name eq 'Photo Library';
  ...
  foreach my $album (@albums) {
    ...
    my $uri = URI->new("http://www.example.com:8770/databases/1/containers/$albumid/items?meta=dpap.aspectratio,dpap.imagefilesize,dpap.creationdate&type=photo");
	...
	my $thumbnail_url = $self->construct_uri("http://www.example.com:8770/databases/1/items?meta=dpap.thumb&query=('dmap.itemid:$imageid')");
	$image->thumbnail_url($thumbnail_url);

	my $hires_url = $self->construct_uri("http://www.example.com:8770/databases/1/items?meta=dpap.hires&query=('dmap.itemid:$imageid')");
...
sub do_get {
  ...
  my $path = shift;
  my $ua = $self->ua;
  my $uri = $self->construct_uri($path);
...
sub construct_uri {
  ...
  $form{"session-id"} = $session_id if $session_id;
  $uri->query_form(%form) if $session_id;
  $uri =~ s/%3A/:/; # mmm, non-standard-compliant
  return $uri;
}
